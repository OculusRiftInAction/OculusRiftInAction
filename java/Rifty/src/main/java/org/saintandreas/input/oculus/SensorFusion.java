...
import org.saintandreas.input.oculus.tracker.TrackerSample;
import org.saintandreas.input.oculus.tracker.TrackerVector;
...
public class SensorFusion {
  ...
  public void processSensorMessage() {
    ...
    if (AngVL > 0.0f) {
      Q.multLocal(new Quaternion().fromAngleAxis(AngVL * DeltaT, AngV.divide(AngVL)));
    }
  ...
  private void tiltCorrect() {
    ...
    if (TiltErrorAngle > MIN_TILT_ERROR) {
      if ((TiltErrorAngle > 0.4f) && (RunningTime < 8.0f)) {
        ...
        Q.multLocal(new Quaternion().fromAngleAxis(-TiltErrorAngle, TiltErrorAxis));
        TiltErrorAngle = 0.0f;
      } else {
        ...
        Q.multLocal(new Quaternion().fromAngleAxis(deltaTiltAngle, TiltErrorAxis));
  ...
  public synchronized Quaternion getPredictedOrientation(float pdt) {
    ...
    if (predictionEnabled) {
      ...
      if (angVelFL > 0.001f) {
        qP.multLocal(new Quaternion().fromAngleAxis(angVelFL * newpdt, angVelF.divide(angVelFL)));
      }
    }
    return qP;
  }
