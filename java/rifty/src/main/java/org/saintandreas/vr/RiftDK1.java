...
public class RiftDK1 extends Hmd {
  ...
  public Vector2f getDistorted(Eye eye, Vector2f screen) {
    ...
    Vector2f rift = new Vector2f(screen.x + xoffset, screen.y * yscale);
    double length = rift.length();
    double distortion = findDistortedRadius(length);
    Vector2f distorted = rift.normalize().mult((float)distortion);
    @SuppressWarnings("unused")
    Vector2f restored = distorted.mult((float)getRiftDistortion(distorted.length()));
    distorted = new Vector2f(distorted.x - xoffset, distorted.y / yscale);
    return distorted;
  }
