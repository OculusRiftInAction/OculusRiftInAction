...
class Example04 : public GlfwApp {
protected:
...
public:
  ...
  void initGl() {
    ...
    float aspect = (float) eyeSize.x / (float) eyeSize.y;
    ...
    float distortionPostScale = 1.0f / stereoConfig.GetDistortionScale();
    vec4 K = vec4(hmdInfo.DistortionK[0], hmdInfo.DistortionK[1],
        hmdInfo.DistortionK[2], hmdInfo.DistortionK[3]);

    distortProgram->use();
    GL_CHECK_ERROR;
    distortProgram->setUniform("LensCenter", lensOffset);
    GL_CHECK_ERROR;
