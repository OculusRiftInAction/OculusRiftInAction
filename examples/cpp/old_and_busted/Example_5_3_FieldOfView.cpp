...
public:
  ShaderLookupDistort() : lookupTextureSize(512, 512), chroma(true) {
    ...
    float ovrScale = stereoConfig.GetDistortionScale();

    RiftDistortionHelper helper(ovrHmdInfo);
    float myScale = helper.getUndistortionScaleForRadius(1);
    myScale /= ovrScale;
    myScale = helper.getUndistortionScaleForRadius(1 + stereoConfig.GetProjectionCenterOffset());
