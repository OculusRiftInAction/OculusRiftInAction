...
public:
  Size boardSize{ 9, 6 };            // The size of the board -> Number of items by width and height
  Pattern calibrationPattern{ CHESSBOARD };// One of the Chessboard, circles, or asymmetric circle pattern
  float squareSize{ 24.0f / 1000.0f };          // The size of a square in your defined unit (point, millimeter,etc).
  int nrFrames{ 5 };              // The number of frames to use from the input for calibration
  float aspectRatio{ 1.0 };         // The aspect ratio
  int delay{ 500 };                 // In case of a video input
  bool bwritePoints{ true };         //  Write detected feature points
  bool bwriteExtrinsics{ true };     // Write extrinsic parameters
